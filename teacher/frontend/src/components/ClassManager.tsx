...
const ClassManager = () => {
  const [classes, setClasses] = useState<Workshop[]>([]);
    ...
    const { error } = await supabase
      ...
      .insert([
        { 
          ...
          term: '2025-T1' // Required by schema
        }
      ]);
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white p-6 rounded-lg shadow-md">
        ...
        <form onSubmit={handleCreate} className="flex gap-4 items-end">
          ...
          <div className="w-32">
            <label className="block text-sm font-medium text-gray-700">Code/Grade</label>
            <input
              type="text"
              value={grade}
      ...
      {/* Class Grid */}
      <div>
        <h2 className="text-2xl font-bold text-gray-900 mb-6">Your Workshops</h2>
        {classes.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {classes.map((cls) => (
              <Link key={cls.workshop_id} href={`/classes/${cls.workshop_id}`}>
